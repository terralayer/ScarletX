...
def test_release_is_locked_to_048_everywhere():
...
def test_truenas_metadata_and_compose_are_locked_to_048():
...
def test_main_container_publishing_never_overwrites_stable_release_tags():
    ...
    assert "type=raw,value=0.4.8" not in workflow
...
def test_release_workflow_selects_only_locked_048_and_tags_tested_head():
    workflow = text(".github/workflows/release.yml")
    assert "workflow_dispatch:" in workflow
    assert "NEXT_VERSION=\"0.4.8\"" in workflow
    assert "Select locked release version" in workflow
    assert "Apply locked 0.4.8 release metadata" in workflow
...
def test_release_helper_rejects_versions_after_048():
    module = load_release_version_module()
    assert module.LOCKED_RELEASE_VERSION == LOCKED_VERSION
    assert module.next_patch_version("0.4.7") == LOCKED_VERSION
    assert module.next_release_version("0.4.8-beta.1") == LOCKED_VERSION

    for current in ("0.4.8", "0.4.9", "0.4.10", "0.4.99"):
        with pytest.raises(ValueError, match="locked at 0.4.8"):
            module.next_patch_version(current)

    for current in ("0.4.9-beta.1", "0.4.10-beta.2"):
        with pytest.raises(ValueError, match="locked at 0.4.8"):
...
def test_release_notes_stay_out_of_runtime_backend_image():
    ...
    assert (ROOT / "RELEASE-NOTES-0.4.8.md").exists()
