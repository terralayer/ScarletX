...
def test_downloaded_scene_page_filters_metadata_only_and_missing_scenes(tmp_path):
    ...
    with factory() as db:
        downloaded = Scene(tpdb_id="downloaded", title="Downloaded", content_type="scene", monitored=True)
        wanted = Scene(tpdb_id="wanted", title="Wanted", content_type="scene", monitored=True)
        unmonitored = Scene(tpdb_id="unmonitored", title="Unmonitored", content_type="scene", monitored=False)
        missing = Scene(tpdb_id="missing", title="Missing", content_type="scene")
        db.add_all([downloaded, wanted, unmonitored, missing])
        db.flush()
        good_media = MediaFile(scene_id=downloaded.id, path=str(tmp_path / "downloaded.mp4"), size_bytes=5)
        unmonitored_media = MediaFile(scene_id=unmonitored.id, path=str(tmp_path / "unmonitored.mp4"), size_bytes=5)
        missing_media = MediaFile(scene_id=missing.id, path=str(tmp_path / "missing.mp4"), size_bytes=5)
        db.add_all([good_media, unmonitored_media, missing_media])
        db.flush()
