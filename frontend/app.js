...
async function render(){nav();if(view==='dashboard')return dashboard();if(view==='library')return mediaLibrary();if(view==='search-choice')return searchChoice();if(['scenes','performers','studios'].includes(view))return renderEntities(view);if(view==='wanted')return wanted();if(view==='activity')return activity();if(view==='calendar')return calendar();if(view==='settings')return settings()}
...
async function renderEntities(type,forcedQuery=''){
  ...
  let pageSizePicker=mode==='library'?`<label class="entity-page-size">Rows per page<select id="entityPageSize" aria-label="Rows per page">${[25,50,100].map(size=>`<option value="${size}" data-page-size="${size}" ${entityPageSize[type]===size?'selected':''}>${size}</option>`).join('')}</select></label>`:'';
  $('#app').innerHTML=pageHead(names[type],'',pageSizePicker)+`<div class="toolbar">${libraryFilters}${mode==='library'&&type==='scenes'?`<button class="btn" id="refreshAll">Refresh Library</button>`:''}</div><div id="entityNotice"></div><div class="${type==='scenes'?'':'cardgrid'}" id="entityGrid"></div>`;
...
function bindEntityPager(type,data){
  const grid=$('#entityGrid'),sourcePage=entityPage[type];
  grid?.querySelectorAll('[data-entity-page]').forEach(button=>button.onclick=()=>{
...
function paintEntityLibrary(type,data,append=false){
  ...
  grid.insertAdjacentHTML('beforeend',entityPager(type,total,!!data.has_more));
