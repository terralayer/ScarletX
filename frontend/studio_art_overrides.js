studioLink=function(x){
  ...
  let logo=id?`<span class="studio-logo"><img src="/api/artwork/studios/${encodeURIComponent(id)}" alt="" loading="lazy" onerror="this.closest('.studio-logo').remove()"></span>`:'';
...
entityCard=function(type,x,inLibrary){
  ...
  let cachedImg=type==='performers'?`/api/artwork/performers/${encodeURIComponent(id)}?size=card`:type==='studios'?`/api/artwork/studios/${encodeURIComponent(id)}`:img;
