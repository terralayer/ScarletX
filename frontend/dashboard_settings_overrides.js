dashboard=async function(){
  ...
  try{
    ...
    $('#stats').innerHTML=[
      ['▣','Scenes',recent.total||0,'Downloaded'],
    ...
    ].map(x=>`<div class="stat"><div class="stat-icon">${x[0]}</div><div><small>${x[1]}</small><strong>${x[2]}</strong><em>${x[3]}</em></div></div>`).join('');
