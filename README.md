...
Screenshots of the current interface with fictional sample data and artwork omitted.

**Dashboard — Recent Scenes and Upcoming Releases share aligned rows.**

![ScarletX dashboard](docs/images/scarletx-dashboard.png)

**Scenes — library, monitoring status, and playback actions.**
...
Container releases always use matching versions for both images, for example ScarletX 0.5.0:

```text
ghcr.io/terralayer/scarletx:0.5.0
ghcr.io/terralayer/scarletx-web:0.5.0
```
...
Stable releases are created with the manual `Release ScarletX` GitHub Actions workflow. The release helper enforces the `0.5.x` series and currently locks releases at `0.5.0`. Advancing to a new version requires deliberately updating that lock; published stable tags are immutable. Adding or changing the workflow does not create a release; a release occurs only when the workflow is manually dispatched with release notes.

Current application version: **0.5.0**.

See [RELEASE-NOTES-0.5.0.md](RELEASE-NOTES-0.5.0.md) for the current release summary. Main-branch changes after that release are available in the matching `:main` container images.
...
npm run screenshots --prefix tools/browser
```

The browser check loads the real frontend with isolated API fixtures and measures panel and row alignment at desktop, tablet, and mobile widths, including long titles, empty lists, and unequal item counts. No running server or personal library is required.
