...
from ..dashboard_data import downloaded_scene_page, recent_performers, recent_studios
...
def dashboard_performers_runtime(
    limit: int = Query(8, ge=1, le=50),
    db: Session = Depends(get_session),
):
    return {"items": recent_performers(db, limit=limit)}
