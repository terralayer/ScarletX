...
class SegmentFetcher:
    def __init__(self, providers: list[UsenetProviderConfig], max_retries: int):
        ...
        self.perf = {
            p.name: {"samples": 0, "bytes": 0, "seconds": 0.0, "ewma_bps": 0.0, "failures": 0, "missing": 0}
    ...
    def _provider_order(self, segment: NZBSegment) -> list[UsenetProviderConfig]:
        ...
        with self.schedule_lock, self.stats_lock:
            ...
            rest = sorted((p for p in self.providers if p.name != primary.name), key=lambda p: (-(self.perf[p.name]["ewma_bps"] or 0.0), p.priority))
            return [primary, *rest]
    ...
    def provider_stats(self) -> list[dict]:
        with self.stats_lock:
            return [{"name": p.name,"samples": int(self.perf[p.name]["samples"]),"speed_bps": round(float(self.perf[p.name]["ewma_bps"] or 0.0), 2),"failures": int(self.perf[p.name]["failures"]),"missing": int(self.perf[p.name]["missing"]),"connections": p.connections} for p in self.providers]

    def abort_active(self) -> None:
        for pool in self.pools.values():
            try: pool.abort_active()
    ...
    def fetch(self, segment: NZBSegment, path: Path) -> DecodedSegment:
        ...
        while attempts < attempt_budget:
            ...
            provider = None; connection = None
            for candidate in candidates:
                leased = self.pools[candidate.name].try_acquire()
                if leased is not None:
                    provider, connection = candidate, leased; break
            if provider is None or connection is None:
                time.sleep(0.01); continue
            attempted_once.add(provider.name); attempts += 1
            pool = self.pools[provider.name]; broken = False; started = time.monotonic()
            try:
                ...
                self._record_failure(provider, missing=True, segment=segment); errors.append(str(exc)); unavailable.add(provider.name)
            except Exception as exc:
                self._record_failure(provider, segment=segment); errors.append(f"{provider.name} attempt {attempts}: {exc}"); broken = True
            finally: pool.release(connection, broken=broken)
            if attempts >= len(self.providers) and attempts < attempt_budget:
                retry_no = attempts - len(self.providers) + 1; time.sleep(min(2.0, 0.15 * (2 ** max(0, retry_no - 1))))
        raise NativeUsenetError("; ".join(errors[-8:]) or "Article download failed")

    def fetch_into(self, segment: NZBSegment, target: Path, done_marker: Path, *, job_id: str | None = None) -> DecodedSegment:
        done_size = _read_done_marker(done_marker)
        if done_size > 0 and target.exists(): return DecodedSegment(path=target, size=done_size, filename=None, provider=None)
        errors: list[str] = []; unavailable: set[str] = set(); attempted_once: set[str] = set(); attempt_budget = max(1, len(self.providers) + self.max_retries); attempts = 0
        while attempts < attempt_budget:
            order = [p for p in self._provider_order(segment) if p.name not in unavailable]
            if not order: break
            candidates = [p for p in order if p.name not in attempted_once] or order
            ...
            for candidate in candidates:
                leased = self.pools[candidate.name].try_acquire()
                if leased is not None: provider, connection = candidate, leased; break
            if provider is None or connection is None: time.sleep(0.004); continue
            attempted_once.add(provider.name); attempts += 1; pool = self.pools[provider.name]; broken = False; started = time.monotonic()
            try:
                    ...
                    try:
                        with download_phase_metrics.start(job_id, "receive"):
                            with download_phase_metrics.start(job_id, "decode_write"):
                                size, filename, begin, total_size = decode_yenc_native(connection, segment, self._target_writer(target))
                    except (AttributeError, TypeError, BufferError, NotImplementedError) as native_exc:
                        self.native_acceleration = False; self.native_acceleration_error = str(native_exc)[:500]; self.close_target(target); broken = True
                        raise NativeUsenetError(f"SIMD yEnc acceleration unavailable at runtime; retrying with built-in decoder: {native_exc}") from native_exc
                else:
                    with download_phase_metrics.start(job_id, "receive"):
                        with download_phase_metrics.start(job_id, "decode_write"):
                            size, filename, begin, total_size = decode_yenc_to_target(connection.body_iter(segment.message_id), target, write_lock=self._target_lock(target))
                _write_done_marker(done_marker, size, begin, total_size); self._record_success(provider, size, time.monotonic() - started, segment)
                return DecodedSegment(path=target,size=size,filename=filename,provider=provider.name,begin=begin,total_size=total_size)
            except FileNotFoundError as exc:
                self._record_failure(provider, missing=True, segment=segment); errors.append(str(exc)); unavailable.add(provider.name)
            except Exception as exc:
                self._record_failure(provider, segment=segment); errors.append(f"{provider.name} attempt {attempts}: {exc}"); broken = True
            ...
            if attempts >= len(self.providers) and attempts < attempt_budget:
                retry_no = attempts - len(self.providers) + 1; time.sleep(min(1.0, 0.08 * (2 ** max(0, retry_no - 1))))
        raise NativeUsenetError("; ".join(errors[-8:]) or "Article download failed")

def _tool_status() -> dict:
    return {"par2": shutil.which("par2") or shutil.which("par2repair") or shutil.which("par2verify"),"unrar": shutil.which("unrar") or shutil.which("unrar-free") or shutil.which("unar"),"7z": shutil.which("7z") or shutil.which("7zz"),"sabctools": bool(sabctools)}


def tool_status() -> dict:
    tools = _tool_status(); return {key: bool(value) for key, value in tools.items()}


_ACTIVE_TOOLS_LOCK = threading.RLock(); _ACTIVE_TOOLS: dict[str, subprocess.Popen] = {}; _CANCELLED_JOBS_LOCK = threading.RLock(); _CANCELLED_JOBS: set[str] = set()


def _postprocess_cancelled(job_id: str | None) -> bool:
    if not job_id: return False
    with _CANCELLED_JOBS_LOCK: return job_id in _CANCELLED_JOBS
...
def _run_tool(command: list[str], cwd: Path, timeout: int, *, job_id: str | None = None, label: str = "Post-processing tool") -> subprocess.CompletedProcess:
    if _postprocess_cancelled(job_id): raise asyncio.CancelledError
    proc = subprocess.Popen(command,cwd=str(cwd),stdin=subprocess.DEVNULL,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True)
    if job_id:
        with _ACTIVE_TOOLS_LOCK: _ACTIVE_TOOLS[job_id] = proc
    try:
        try: stdout, _ = proc.communicate(timeout=timeout)
        except subprocess.TimeoutExpired:
            proc.kill(); stdout, _ = proc.communicate(); raise NativeUsenetError(f"{label} timed out after {timeout // 60} minutes")
        if _postprocess_cancelled(job_id): raise asyncio.CancelledError
        return subprocess.CompletedProcess(command, proc.returncode, stdout, None)
    finally:
        if job_id:
            with _ACTIVE_TOOLS_LOCK:
                if _ACTIVE_TOOLS.get(job_id) is proc: _ACTIVE_TOOLS.pop(job_id, None)


def repair_payload(payload_dir: Path, enabled: bool, *, job_id: str | None = None) -> list[str]:
    notes: list[str] = []; par_files = sorted(payload_dir.rglob("*.par2"))
    if not par_files: return notes
    if not enabled: notes.append("PAR2 files present; repair disabled"); return notes
    tools = _tool_status(); tool = tools["par2"]
    if not tool: notes.append("PAR2 files present; par2 tool is not installed"); return notes
    primary = _primary_par2_file(payload_dir) or par_files[0]; base = Path(tool).name.casefold()
    if "verify" in base:
        verify_cmd = [tool, str(primary)]; repair_tool = shutil.which("par2repair"); repair_cmd = [repair_tool, str(primary)] if repair_tool else None
    elif "repair" in base:
        verify_tool = shutil.which("par2verify"); verify_cmd = [verify_tool, str(primary)] if verify_tool else [tool, str(primary)]; repair_cmd = [tool, str(primary)]
    else: verify_cmd = [tool, "v", str(primary)]; repair_cmd = [tool, "r", str(primary)]
    with download_phase_metrics.start(job_id, "verify"): verified = _run_tool(verify_cmd, payload_dir, 180, job_id=job_id, label="PAR2 verification")
    if verified.returncode == 0: notes.append("PAR2 verification passed"); return notes
    if not repair_cmd: raise NativeUsenetError("PAR2 verification failed and no repair command is available")
    with download_phase_metrics.start(job_id, "repair"): repaired = _run_tool(repair_cmd, payload_dir, 600, job_id=job_id, label="PAR2 repair")
    if repaired.returncode != 0: tail = (repaired.stdout or verified.stdout or "PAR2 repair failed")[-1200:]; raise NativeUsenetError(f"PAR2 repair failed: {tail}")
    notes.append("PAR2 repair completed"); return notes


def _main_rars(payload_dir: Path) -> list[Path]:
    rars = sorted(payload_dir.rglob("*.rar")); result = []
    for item in rars:
        name = item.name.casefold()
        if re.search(r"\.part\d+\.rar$", name) and not re.search(r"\.part0*1\.rar$", name): continue
        result.append(item)
    return result


def _archive_files(payload_dir: Path) -> list[Path]: return [*sorted(payload_dir.rglob("*.zip")), *_main_rars(payload_dir), *sorted(payload_dir.rglob("*.7z"))]


def unpack_payload(payload_dir: Path, enabled: bool, password: str = "", *, job_id: str | None = None) -> list[str]:
    notes: list[str] = []
    if not enabled: return notes
    for archive in sorted(payload_dir.rglob("*.zip")):
        if _postprocess_cancelled(job_id): raise asyncio.CancelledError
        try:
            deadline = time.monotonic() + 600; root = archive.parent.resolve()
            with zipfile.ZipFile(archive) as zf:
                for member in zf.infolist():
                    if _postprocess_cancelled(job_id): raise asyncio.CancelledError
                    if time.monotonic() > deadline: raise NativeUsenetError(f"Extracting {archive.name} timed out after 10 minutes")
                    try: safe_member = validate_archive_member_path(member.filename)
                    except ValueError as exc: raise NativeUsenetError(f"Unsafe path in ZIP archive {archive.name}") from exc
        ...
        except zipfile.BadZipFile as exc: raise NativeUsenetError(f"Could not unpack {archive.name}: {exc}") from exc
        except RuntimeError as exc: raise NativeUsenetError(f"Could not unpack {archive.name}: {exc}") from exc
    rars = _main_rars(payload_dir); sevens = sorted(payload_dir.rglob("*.7z"))
    if not rars and not sevens: return notes
    tools = _tool_status(); archive_tool = tools["7z"]
    if not archive_tool: raise NativeUsenetError("Secure RAR/7z extraction requires 7z")
    for archive in [*rars, *sevens]:
        if _postprocess_cancelled(job_id): raise asyncio.CancelledError
        listing = [archive_tool, "l", "-slt"]
        if password: listing.append(f"-p{password}")
        listing.append(str(archive)); listed = _run_tool(listing, archive.parent, 120, job_id=job_id, label=f"Listing {archive.name}")
        if listed.returncode != 0: raise NativeUsenetError(f"Could not inspect {archive.name}: {(listed.stdout or '')[-1200:]}")
        try: members = parse_7z_listing(listed.stdout or "")
        except ValueError as exc: raise NativeUsenetError(f"Unsafe path in archive {archive.name}: {exc}") from exc
        if not members: raise NativeUsenetError(f"Archive {archive.name} contains no safely listable members")
        with TemporaryDirectory(prefix=".scarletx-extract-", dir=archive.parent) as temp_dir:
            quarantine = Path(temp_dir); command = [archive_tool, "x", "-y"]
            if password: command.append(f"-p{password}")
            command.extend([f"-o{quarantine}", str(archive)]); result = _run_tool(command, archive.parent, 600, job_id=job_id, label=f"Extracting {archive.name}")
            if result.returncode != 0: raise NativeUsenetError(f"Could not unpack {archive.name}: {(result.stdout or '')[-1200:]}")
            try: validate_extracted_tree(quarantine)
            except ValueError as exc: raise NativeUsenetError(f"Unsafe extracted content in {archive.name}: {exc}") from exc
            for child in quarantine.iterdir():
                target = archive.parent / child.name; target = _unique_directory(target) if child.is_dir() else _unique_file_path(target); shutil.move(str(child), str(target))
        notes.append(f"Extracted {archive.name}")
    return notes


def postprocess_payload(payload_dir: Path, *, repair_enabled: bool, unpack_enabled: bool, password: str = "", job_id: str | None = None, stage=None) -> tuple[list[str], list[Path]]:
    notes: list[str] = []
    def mark(message: str) -> None:
        if _postprocess_cancelled(job_id): raise asyncio.CancelledError
        if stage: stage(message)
    mark("Detecting downloaded payloads"); notes.extend(normalize_obfuscated_payload(payload_dir)); videos = _playable_videos(payload_dir)
    if not videos:
        mark("Identifying primary media")
        with download_phase_metrics.start(job_id, "probe"): notes.extend(recover_unknown_videos(payload_dir))
        videos = _playable_videos(payload_dir)
    archives = _archive_files(payload_dir); par_present = _primary_par2_file(payload_dir) is not None
    if videos and (not archives or videos[0].stat().st_size >= 32 * 1024 * 1024):
        if par_present and repair_enabled: notes.append("Playable scene detected; skipped unnecessary PAR2 verification")
        if archives and unpack_enabled: notes.append("Playable scene detected; skipped unnecessary archive extraction")
        return notes, videos
    if par_present:
        mark("Checking PAR2 integrity"); notes.extend(repair_payload(payload_dir, repair_enabled, job_id=job_id)); notes.extend(normalize_obfuscated_payload(payload_dir))
    ...
    if archives and not videos:
        ...
        with download_phase_metrics.start(job_id, "extract"): notes.extend(unpack_payload(payload_dir, unpack_enabled, password, job_id=job_id))
        mark("Identifying extracted media"); notes.extend(normalize_obfuscated_payload(payload_dir))
        if not _playable_videos(payload_dir):
            with download_phase_metrics.start(job_id, "probe"): notes.extend(recover_unknown_videos(payload_dir))
        videos = _playable_videos(payload_dir)
    return notes, videos
...
_LIVE_PROGRESS_LOCK = threading.RLock(); _LIVE_PROGRESS: dict[str, dict[str, float | int | None]] = {}; _PROGRESS_FIELDS = ("total_bytes", "downloaded_bytes", "speed_bps", "eta_seconds"); _PROGRESS_CHECKPOINT_GATE = ProgressCheckpointGate(interval_seconds=2.0,byte_threshold=8 * 1024 * 1024)


def _set_live_progress(job_id: str, **values) -> None:
    with _LIVE_PROGRESS_LOCK: _LIVE_PROGRESS.setdefault(job_id, {}).update(values)

def _get_live_progress(job_id: str) -> dict:
    with _LIVE_PROGRESS_LOCK: return dict(_LIVE_PROGRESS.get(job_id, {}))

def _clear_live_progress(job_id: str) -> None:
    with _LIVE_PROGRESS_LOCK: _LIVE_PROGRESS.pop(job_id, None)
    _PROGRESS_CHECKPOINT_GATE.clear(job_id)

def apply_live_progress(job: NativeUsenetJob) -> bool:
    live = _get_live_progress(job.id); changed = False
    for field in _PROGRESS_FIELDS:
        if field in live: setattr(job, field, live[field]); changed = True
    return changed

@event.listens_for(NativeUsenetJob.status, "set")
def _apply_live_progress_on_status_set(job, value, oldvalue, _initiator) -> None:
    if value != oldvalue: apply_live_progress(job)
...
def _apply_live_progress_on_cancel_set(job, value, oldvalue, _initiator) -> None:
    if value and value != oldvalue: apply_live_progress(job)


def enqueue_url(session_factory, settings, url: str, title: str) -> str:
    if not native_client_ready(settings): raise NativeUsenetError("ScarletX built-in Usenet has no enabled provider configured")
    with session_factory() as db:
        existing = db.scalar(select(NativeUsenetJob).where(NativeUsenetJob.nzb_url == url,NativeUsenetJob.status.in_(["queued", "downloading", "paused", "postprocessing"])).order_by(NativeUsenetJob.created_at.desc()).limit(1))
        if existing: return existing.id
        job_id = "sx-" + uuid.uuid4().hex; db.add(NativeUsenetJob(id=job_id, title=title, nzb_url=url, status="queued")); db.commit()
    return job_id
...
async def reprocess_completed_job(session_factory, settings, job_id: str) -> dict:
    with session_factory() as db:
        ...
        if job is None: raise NativeUsenetError("Built-in download job not found")
        output_path = Path(job.output_path).expanduser().resolve() if job.output_path else None; password = job.unpack_password or ""
    if output_path is None or not output_path.exists() or not output_path.is_dir(): raise NativeUsenetError("Completed payload directory is missing")
    with _CANCELLED_JOBS_LOCK: _CANCELLED_JOBS.discard(job_id)
    def stage(message: str) -> None: _set_job(session_factory, job_id, postprocess_note=message)
    notes, videos = await asyncio.to_thread(postprocess_payload,output_path,repair_enabled=settings.native_usenet_repair_enabled,unpack_enabled=settings.native_usenet_unpack_enabled,password=password,job_id=job_id,stage=stage)
    if not videos:
        names = ", ".join(p.name for p in sorted(output_path.rglob("*")) if p.is_file())[:1200]; raise NativeUsenetError("No playable scene was found after reprocessing" + (f": {names}" if names else ""))
    primary = videos[0]; notes.append(f"Primary scene: {primary.name} ({primary.stat().st_size} bytes)")
    _set_job(session_factory, job_id, status="completed", error=None, postprocess_note="; ".join(notes)[:2000] if notes else f"Primary scene: {primary.name}", completed_at=utcnow(), unpack_password=None)
    with session_factory() as db: return job_dict(db.get(NativeUsenetJob, job_id))

def native_client_ready(settings) -> bool: return bool(settings.native_usenet_enabled and [p for p in settings.native_usenet_providers() if p.enabled and p.host])
...
def _set_job(session_factory, job_id: str, **values) -> None:
    ...
    if live_values: _set_live_progress(job_id, **live_values)
    if "status" in values:
        current_live = _get_live_progress(job_id)
        for field in _PROGRESS_FIELDS:
            if field in current_live: values.setdefault(field, current_live[field])
    values["updated_at"] = utcnow()
    with session_factory() as db: db.execute(update(NativeUsenetJob).where(NativeUsenetJob.id == job_id).values(**values)); db.commit()
...
def _publish_progress(session_factory, job_id: str, *, total_bytes: int, downloaded_bytes: int, speed_bps: float, eta_seconds: int | None) -> None:
    values = {"total_bytes": int(total_bytes),"downloaded_bytes": int(downloaded_bytes),"speed_bps": float(speed_bps),"eta_seconds": eta_seconds}; _set_live_progress(job_id, **values)
    if _PROGRESS_CHECKPOINT_GATE.should_persist(job_id, values["downloaded_bytes"], time.monotonic()): _set_job(session_factory, job_id, **values)
...
async def _fetch_nzb(url: str) -> bytes:
    try:
        async with httpx.AsyncClient(timeout=45, follow_redirects=True, headers={"User-Agent": "ScarletX/0.3.10-beta.4"}) as client:
            response = await client.get(url); response.raise_for_status(); payload = response.content
    except httpx.HTTPError as exc: raise NativeUsenetError(f"Could not download NZB: {exc}") from exc
    if not payload.strip(): raise NativeUsenetError("Indexer returned an empty NZB")
    return payload
...
async def process_job(session_factory, settings, job_id: str) -> None:
    ...
    with session_factory() as db:
        job = db.get(NativeUsenetJob, job_id)
        if job is None or job.status not in {"queued", "downloading", "postprocessing", "paused"}: return
        title = job.title; url = job.nzb_url; unpack_password = job.unpack_password or ""
    providers = [p for p in settings.native_usenet_providers() if p.enabled and p.host]
    if not providers:
        _set_job(session_factory, job_id, status="failed", error="No enabled Usenet provider is configured", completed_at=utcnow(), unpack_password=None); return
    incomplete_root = Path(settings.native_usenet_incomplete_dir).expanduser().resolve(); complete_root = Path(settings.native_usenet_complete_dir).expanduser().resolve(); failed_root = incomplete_root.parent / "failed"; work = incomplete_root / job_id
    with session_factory() as db:
        current = db.get(NativeUsenetJob, job_id); prior_path = Path(current.output_path).expanduser() if current and current.output_path else None
    if prior_path and prior_path.exists() and failed_root in prior_path.resolve().parents:
        ...
        incomplete_root.mkdir(parents=True, exist_ok=True); shutil.move(str(prior_path), str(work)); _set_job(session_factory, job_id, output_path=None)
    payload_dir = work / "payload"; state_root = work / "segments"; assembly_root = work / "assembly"; payload_dir.mkdir(parents=True, exist_ok=True); state_root.mkdir(parents=True, exist_ok=True); assembly_root.mkdir(parents=True, exist_ok=True)
    try:
        ...
        if nzb_file.exists() and nzb_file.stat().st_size: nzb_payload = nzb_file.read_bytes()
        else: nzb_payload = await _fetch_nzb(url); nzb_file.write_bytes(nzb_payload)
        files = parse_nzb(nzb_payload); validate_nzb_release_size(files); file_states: dict[int, dict] = {}; primary_indices: list[int] = []; deferred_indices: list[int] = []; optional_indices: list[int] = []
        for file_index, item in enumerate(files, 1):
            if nzb_file_is_ignored(item, file_index): continue
            priority, deferred = _file_download_priority(item, file_index); state_dir = state_root / f"{file_index:04d}"; state_dir.mkdir(parents=True, exist_ok=True); assembly = assembly_root / f"{file_index:04d}.part"; filename_marker = state_dir / "filename.txt"
            legacy = sorted(state_dir.glob("*.seg")); done = sorted(state_dir.glob("*.done"))
                ...
                if len(legacy) == len(item.segments) and all(x.stat().st_size > 0 for x in legacy):
                    with assembly.open("wb", buffering=1024 * 1024) as output:
                        ...
                        for segment, part in zip(item.segments, legacy):
                            ...
                            with part.open("rb") as source: shutil.copyfileobj(source, output, length=4 * 1024 * 1024)
                            _write_done_marker(state_dir / f"{segment.number:06d}.done", size, offset, None); offset += size
                    for part in legacy: part.unlink(missing_ok=True)
                else:
                    for part in legacy: part.unlink(missing_ok=True)
            file_states[file_index] = {"item": item,"priority": priority,"deferred": deferred,"state_dir": state_dir,"assembly": assembly,"filename_marker": filename_marker}
            if deferred: deferred_indices.append(file_index)
            elif priority >= 70: optional_indices.append(file_index)
            else: primary_indices.append(file_index)
        if not file_states: raise NativeUsenetError("Release contains only sample or image payloads")
        if not primary_indices:
            primary_indices = optional_indices or deferred_indices
            if primary_indices is optional_indices: optional_indices = []
            else: deferred_indices = []
            for idx in primary_indices: file_states[idx]["deferred"] = False
        def completed_bytes(indices: list[int]) -> int:
            return sum(_read_done_marker(file_states[idx]["state_dir"] / f"{segment.number:06d}.done") for idx in indices for segment in file_states[idx]["item"].segments)
        def advertised_bytes(indices: list[int]) -> int: return sum(segment.bytes for idx in indices for segment in file_states[idx]["item"].segments)
        def finalize_files(indices: list[int]) -> None:
            for idx in indices:
                state = file_states[idx]; item: NZBFile = state["item"]; state_dir: Path = state["state_dir"]; assembly: Path = state["assembly"]; markers = [state_dir / f"{segment.number:06d}.done" for segment in item.segments]
                if not all(_read_done_marker(marker) > 0 for marker in markers): raise NativeUsenetError(f"Missing segments for NZB file {idx}")
                filename_marker: Path = state["filename_marker"]
                if filename_marker.exists(): filename = _safe_filename(filename_marker.read_text(errors="ignore"), f"file-{idx:04d}.bin")
                else: filename = _safe_filename(_subject_filename(item.subject, idx), f"file-{idx:04d}.bin"); filename_marker.write_text(filename)
                destination = payload_dir / filename; fetcher.close_target(assembly)
                if destination.exists() and not assembly.exists(): continue
                if destination.exists() and assembly.exists(): destination = payload_dir / f"{idx:04d}-{filename}"
                if not assembly.exists(): raise NativeUsenetError(f"Assembled payload missing for NZB file {idx}")
                assembly.replace(destination)
        effective_total = advertised_bytes(primary_indices); existing = completed_bytes(primary_indices); downloaded = existing; session_downloaded = 0; transfer_started = time.monotonic(); speed_samples = deque([(transfer_started, 0)], maxlen=256); last_control_check = 0.0; _set_job(session_factory, job_id, total_bytes=effective_total, downloaded_bytes=downloaded)
        hard_cap = min(max(1, int(settings.native_usenet_max_connections)),max(1, sum(p.connections for p in providers)),200)
        ...
        with _ACTIVE_FETCHERS_LOCK: _ACTIVE_FETCHERS[job_id] = fetcher
        ...
        async def download_indices(indices: list[int], *, recovery: bool = False) -> None:
            ...
            for idx in indices:
                state = file_states[idx]; item: NZBFile = state["item"]
                for segment in item.segments:
                    done_marker = state["state_dir"] / f"{segment.number:06d}.done"
                    if _read_done_marker(done_marker) > 0 and (state["assembly"].exists() or (state["filename_marker"].exists() and (payload_dir / _safe_filename(state["filename_marker"].read_text(errors="ignore"), f"file-{idx:04d}.bin")).exists())): continue
            ...
            if not jobs: finalize_files(indices); return
            loop = asyncio.get_running_loop(); position = 0; result_buffer = SegmentResultBuffer(hard_cap); inflight: dict[int, tuple[asyncio.Future, int, Path]] = {}; tune_time = time.monotonic(); tune_speed = 0.0; stable_rounds = 0
            def fetch_and_buffer(token: int, segment: NZBSegment, target: Path, done_marker: Path) -> None:
                try: result = fetcher.fetch_into(segment, target, done_marker, job_id=job_id)
                except BaseException as exc: result_buffer.put((token, None, exc))
                else: result_buffer.put((token, result, None))
            def submit_until_window() -> None:
                nonlocal position
                while len(inflight) < active_window and position < len(jobs):
                    _, idx, _, segment, target, done_marker, filename_marker = jobs[position]; token = position; position += 1; future = loop.run_in_executor(executor, fetch_and_buffer, token, segment, target, done_marker); inflight[token] = (future, idx, filename_marker)
            submit_until_window()
            while inflight:
                now = time.monotonic()
                if now - last_control_check >= 0.35: await _wait_if_paused(session_factory, job_id); last_control_check = now
                try: token, result, error = result_buffer.get_nowait()
                except queue.Empty: await asyncio.sleep(0.02)
                else:
                    future, idx, filename_marker = inflight.pop(token); await future
                    if error is not None: raise error
                    downloaded += result.size; session_downloaded += result.size
                    if result.filename and not filename_marker.exists():
                        try: temp = filename_marker.with_name("filename.txt.tmp"); temp.write_text(_safe_filename(result.filename, f"file-{idx:04d}.bin")); temp.replace(filename_marker)
                        except Exception: pass
                now = time.monotonic(); speed_samples.append((now, session_downloaded))
                while len(speed_samples) > 2 and now - speed_samples[0][0] > 4.0: speed_samples.popleft()
                first_t, first_bytes = speed_samples[0]; speed = max(0.0, (session_downloaded - first_bytes) / max(0.001, now - first_t)); eta = int(max(0, effective_total - downloaded) / speed) if speed > 0 else None
                if now - tune_time >= 1.5:
                    active_window, stable_rounds = _tune_active_window(active_window=active_window,hard_cap=hard_cap,speed=speed,tune_speed=tune_speed,stable_drop_rounds=stable_rounds)
                    tune_speed = max(speed, tune_speed * 0.985); tune_time = now
                _publish_progress(session_factory, job_id,total_bytes=effective_total, downloaded_bytes=downloaded,speed_bps=speed, eta_seconds=eta)
                _set_live_progress(job_id,provider=fetcher.best_provider(),provider_stats=fetcher.provider_stats(),active_connections=active_window,connection_cap=hard_cap,phase="recovery" if recovery else "payload",buffered_segments=result_buffer.qsize(),max_buffered_segments=result_buffer.peak_size)
                limit = float(settings.native_usenet_speed_limit_mb_s or 0)
                if limit > 0 and session_downloaded > 0:
                    target_elapsed = session_downloaded / (limit * 1024 * 1024); actual_elapsed = time.monotonic() - transfer_started
                    if target_elapsed > actual_elapsed: await asyncio.sleep(target_elapsed - actual_elapsed)
                submit_until_window()
            finalize_files(indices); final_now = time.monotonic(); first_t, first_bytes = speed_samples[0]; final_speed = max(0.0, (session_downloaded - first_bytes) / max(0.001, final_now - first_t)); final_eta = int(max(0, effective_total - downloaded) / final_speed) if final_speed > 0 else None; _PROGRESS_CHECKPOINT_GATE.force(job_id); _publish_progress(session_factory, job_id,total_bytes=effective_total, downloaded_bytes=downloaded,speed_bps=final_speed, eta_seconds=final_eta)
        try:
            await download_indices(primary_indices)
            _set_job(session_factory, job_id, status="postprocessing", speed_bps=0.0, eta_seconds=0, postprocess_note="Fast-path media detection"); notes: list[str] = []
            if optional_indices: skipped_support = advertised_bytes(optional_indices); notes.append(f"Skipped {skipped_support / 1024 / 1024:.1f} MB of nonessential NFO/SFV/image support files")
            best_name = fetcher.best_provider()
            if best_name:
                best_stat = next((x for x in fetcher.provider_stats() if x["name"] == best_name), None)
                if best_stat and best_stat.get("speed_bps"): notes.append(f"Fastest provider: {best_name} ({float(best_stat['speed_bps']) / 1024 / 1024:.1f} MB/s/article)")
                else: notes.append(f"Fastest provider: {best_name}")
            def post_stage(message: str) -> None: prefix = (notes[0] + "; ") if notes else ""; _set_job(session_factory, job_id, postprocess_note=(prefix + message)[:2000])
            fast_error: Exception | None = None
            try: fast_notes, videos = await asyncio.to_thread(postprocess_payload,payload_dir,repair_enabled=False,unpack_enabled=settings.native_usenet_unpack_enabled,password=unpack_password,job_id=job_id,stage=post_stage); notes.extend(fast_notes)
            except asyncio.CancelledError: raise
            except Exception as exc: fast_error = exc; videos = []
            if videos and deferred_indices: skipped = advertised_bytes(deferred_indices); notes.append(f"Skipped {skipped / 1024 / 1024:.1f} MB of unnecessary PAR2 recovery volumes")
            elif deferred_indices:
                recovery_total = advertised_bytes(deferred_indices); recovery_existing = completed_bytes(deferred_indices); effective_total += recovery_total; downloaded += recovery_existing; _set_job(session_factory, job_id, status="downloading",total_bytes=effective_total, downloaded_bytes=downloaded,postprocess_note="Fetching PAR2 recovery volumes only because the primary payload needs repair"); await download_indices(deferred_indices, recovery=True); _set_job(session_factory, job_id, status="postprocessing", speed_bps=0.0, eta_seconds=0, postprocess_note="Repairing recovered payload"); post_notes, videos = await asyncio.to_thread(postprocess_payload,payload_dir,repair_enabled=settings.native_usenet_repair_enabled,unpack_enabled=settings.native_usenet_unpack_enabled,password=unpack_password,job_id=job_id,stage=post_stage); notes.extend(post_notes)
            elif not videos:
                post_notes, videos = await asyncio.to_thread(postprocess_payload,payload_dir,repair_enabled=settings.native_usenet_repair_enabled,unpack_enabled=settings.native_usenet_unpack_enabled,password=unpack_password,job_id=job_id,stage=post_stage); notes.extend(post_notes)
            if not videos:
                names = ", ".join(p.name for p in sorted(payload_dir.rglob("*")) if p.is_file())[:1200]; suffix = f"; fast path: {fast_error}" if fast_error else ""; raise NativeUsenetError("Download finished but no playable scene video was found after de-obfuscation, repair, and unpack" + (f": {names}" if names else "") + suffix)
            primary = videos[0]; notes.append(f"Primary scene: {primary.name} ({primary.stat().st_size} bytes)"); complete_root.mkdir(parents=True, exist_ok=True); final_name = _safe_filename(title, job_id); final_dir = _unique_directory(complete_root / final_name); shutil.move(str(payload_dir), str(final_dir)); shutil.rmtree(work, ignore_errors=True); _set_job(session_factory,job_id,status="completed",total_bytes=max(downloaded, effective_total),downloaded_bytes=max(downloaded, effective_total),speed_bps=0.0,eta_seconds=0,output_path=str(final_dir),postprocess_note="; ".join(dict.fromkeys(notes))[:2000] if notes else "Download complete",completed_at=utcnow(),unpack_password=None); _clear_live_progress(job_id)
            with _CANCELLED_JOBS_LOCK: _CANCELLED_JOBS.discard(job_id)
        except BaseException: fetcher.abort_active(); raise
        finally:
            with _ACTIVE_FETCHERS_LOCK: _ACTIVE_FETCHERS.pop(job_id, None)
            executor.shutdown(wait=True, cancel_futures=True); fetcher.close_targets_under(assembly_root)
    except asyncio.CancelledError:
        _set_job(session_factory, job_id, status="cancelled", speed_bps=0.0, eta_seconds=0,output_path=str(work) if work.exists() else None, error=None,postprocess_note="Cancelled; partial data preserved for retry",completed_at=utcnow(), unpack_password=None); _clear_live_progress(job_id)
        with _CANCELLED_JOBS_LOCK: _CANCELLED_JOBS.discard(job_id)
        with session_factory() as db:
            tracked = db.scalar(select(TrackedDownload).where(TrackedDownload.nzo_id == job_id))
            if tracked: tracked.status = "cancelled"; tracked.client_status = "cancelled"; tracked.error = None; tracked.completed_at = utcnow(); tracked.last_checked_at = utcnow(); db.commit()
        task = asyncio.current_task()
        if task is not None and task.cancelling(): raise
    except Exception as exc:
        message = str(exc)[:4000]; failed_path = None
        try:
            failed_root.mkdir(parents=True, exist_ok=True)
            if work.exists(): failed_dir = _unique_directory(failed_root / f"{_safe_filename(title, job_id)} -- {job_id[-8:]}"); shutil.move(str(work), str(failed_dir)); failed_path = str(failed_dir)
        except Exception: failed_path = str(work) if work.exists() else None
        _set_job(session_factory, job_id, status="failed", error=message, speed_bps=0.0, eta_seconds=0, output_path=failed_path, completed_at=utcnow(), unpack_password=None); _clear_live_progress(job_id)
        with _CANCELLED_JOBS_LOCK: _CANCELLED_JOBS.discard(job_id)
        with session_factory() as db:
            tracked = db.scalar(select(TrackedDownload).where(TrackedDownload.nzo_id == job_id))
            if tracked: tracked.status = "failed"; tracked.client_status = "failed"; tracked.error = message; tracked.last_checked_at = utcnow(); db.add(History(event_type="download_failed", scene_id=tracked.scene_id, message=f"Built-in download failed: {tracked.scene_title or tracked.release_title}")); db.commit()
...
async def native_worker_loop(session_factory, settings_loader, poll_seconds: float = 1.0) -> None:
    ...
    with session_factory() as db: db.execute(update(NativeUsenetJob).where(NativeUsenetJob.status.in_(["downloading", "postprocessing"])).values(status="queued", speed_bps=0.0, eta_seconds=None)); db.commit()
    while True:
        with session_factory() as db: job = db.scalar(select(NativeUsenetJob).where(NativeUsenetJob.status == "queued").order_by(NativeUsenetJob.created_at.asc()).limit(1)); job_id = job.id if job else None
        if not job_id: await asyncio.sleep(poll_seconds); continue
        settings = settings_loader(); await process_job(session_factory, settings, job_id)


def queue_rows(db, *, active_only: bool = True, limit: int = 200) -> list[dict]:
    stmt = select(NativeUsenetJob)
    if active_only: stmt = stmt.where(NativeUsenetJob.status.in_(["queued", "downloading", "paused", "postprocessing"]))
    rows = db.scalars(stmt.order_by(NativeUsenetJob.created_at.asc()).limit(limit)).all(); return [job_dict(row) for row in rows]

def completed_rows(db, limit: int = 100) -> list[dict]: return [job_dict(row) for row in db.scalars(select(NativeUsenetJob).where(NativeUsenetJob.status == "completed").order_by(NativeUsenetJob.completed_at.desc()).limit(limit)).all()]

def history_rows(db, limit: int = 100) -> list[dict]: return [job_dict(row) for row in db.scalars(select(NativeUsenetJob).where(NativeUsenetJob.status.in_(["completed", "cancelled"])).order_by(NativeUsenetJob.created_at.desc()).limit(limit)).all()]

def failed_rows(db, limit: int = 200) -> list[dict]: return [job_dict(row) for row in db.scalars(select(NativeUsenetJob).where(NativeUsenetJob.status == "failed").order_by(NativeUsenetJob.updated_at.desc()).limit(limit)).all()]


def job_dict(job: NativeUsenetJob) -> dict:
    live = _get_live_progress(job.id); total = max(0, int(live.get("total_bytes", job.total_bytes or 0) or 0)); done = max(0, int(live.get("downloaded_bytes", job.downloaded_bytes or 0) or 0)); speed_bps = float(live.get("speed_bps", job.speed_bps or 0) or 0); eta_seconds = live.get("eta_seconds", job.eta_seconds); progress = min(100.0, (done / total * 100.0) if total else 0.0)
    return {"id": job.id,"title": job.title,"status": job.status,"total_bytes": total,"downloaded_bytes": done,"progress": round(progress, 2),"speed_bps": speed_bps,"eta_seconds": eta_seconds,"provider": live.get("provider"),"provider_stats": live.get("provider_stats", []),"active_connections": live.get("active_connections"),"connection_cap": live.get("connection_cap"),"phase": live.get("phase"),"buffered_segments": live.get("buffered_segments", 0),"max_buffered_segments": live.get("max_buffered_segments", 0),"phase_metrics": download_phase_metrics.snapshot(job.id),"output_path": job.output_path,"error": job.error,"postprocess_note": job.postprocess_note,"unpack_password_configured": bool(job.unpack_password),"created_at": job.created_at,"started_at": job.started_at,"completed_at": job.completed_at}
