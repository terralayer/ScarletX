...
async def _fetch_nzb(url: str) -> bytes:
    try:
        async with httpx.AsyncClient(timeout=45, follow_redirects=True, headers={"User-Agent": "ScarletX/0.3.10"}) as client:
...
async def process_job(session_factory, settings, job_id: str) -> None:
    ...
    try:
        ...
        # Start with enough parallel BODY requests to fill fast links, then let the
        ...
        async def download_indices(indices: list[int], *, recovery: bool = False) -> None:
            ...
            while inflight:
                ...
                # Usenet article completion is bursty, so grow aggressively through
